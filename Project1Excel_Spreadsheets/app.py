...
wb = xl.load_workbook('/Users/jiazhencai/ProjectLists/PythonLearning/Project1Excel_Spreadsheets/transactions.xlsx') #打开一个 Excel 文件,返回一个 workbook 对象
...
#此处的 values 对象有表单对应范围内的所有单元格的值

chart = BarChart() #创建一个 BarChart的 instance(实例)
chart.add_data(values) #将数据添加到chart实例中（图标中）
sheet.add_chart(chart, 'e2') #将图表添加到工作表中，指定位置为 E2

wb.save('transactions2.xlsx') #保存文件